...
`main` 分支是 MiniHarness 的**最终集成形态**，合并了上述 13 个功能分支的全部能力，是一个可直接使用的生产级 Agent 框架。它不是简单的分支拼接，而是把各模块能力编织成一条完整的数据流：

```
CLI 交互层 → Agent 主循环 → 上下文管理 → Provider 工厂 → 工具调用 → 钩子拦截
     ↕           ↕              ↕            ↕           ↕           ↕
 项目记忆     会话持久化      Token 预算   多模型切换   MCP/子代理   审计/安全
```

- **开箱即用**：`pnpm dev` 即可启动一个具备流式对话、工具调用、上下文管理、安全审批、会话持久化、成本可观测、多模型切换、子代理编排、钩子生命周期、CLI 富文本渲染、项目记忆注入的完整 Agent
- **学习路径终点**：建议先按顺序学习各 `feat/*` 分支理解单层设计，再回到 `main` 研究所有模块如何协同工作
- **评测与回归**：内置 `pnpm dev eval` 基准测试，保障各模块集成后的整体质量
